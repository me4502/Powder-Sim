    ...
    for (i = 0; i<=parts_lastActiveIndex; i++)
    {
        ...
        if (parts[i].type)
        {
            ...
            if (cmode!=CM_HEAT)
            {
                ...
                else if (t==PT_NBLE && parts[i].ctype>=0 && parts[i].ctype < NNBLALT)
                {
                    pixel pc;
                    pc = nmenu[parts[i].ctype].colour;
                    blendpixel(vid, nx, ny, PIXR(pc), PIXG(pc), PIXB(pc), 255);
                    if (cmode==CM_BLOB)
                    {
                        blendpixel(vid, nx+1, ny, PIXR(pc), PIXG(pc), PIXB(pc), 223);
                    ...
                    if (parts[i].ctype == PT_NBLE)
                    {
                        ...
                        else
                        {
                            for (x=-3; x<4; x++)
                            {
                                ...
                                {
                                    if (abs(x)+abs(y) <2 && !(abs(x)==2||abs(y)==2))
                                        blendpixel(vid,x+nx,y+ny, PIXR(nmenu[parts[i].tmp2].colour)/1.6, PIXG(nmenu[parts[i].tmp2].colour)/1.6, PIXB(nmenu[parts[i].tmp2].colour)/1.6, 30);
                                    if (abs(x)+abs(y) <=3 && abs(x)+abs(y))
                                        blendpixel(vid,x+nx,y+ny, PIXR(nmenu[parts[i].tmp2].colour)/1.6, PIXG(nmenu[parts[i].tmp2].colour)/1.6, PIXB(nmenu[parts[i].tmp2].colour)/1.6, 10);
                                    if (abs(x)+abs(y) == 2)
                                        blendpixel(vid,x+nx,y+ny, PIXR(nmenu[parts[i].tmp2].colour)/1.6, PIXG(nmenu[parts[i].tmp2].colour)/1.6, PIXB(nmenu[parts[i].tmp2].colour)/1.6, 20);
                                }
                            }
                        }
                    }
                ...
                else if (t==PT_BIZR||t==PT_BIZRG||t==PT_BIZRS)
                {
                    if (!parts[i].ctype)
                        return 1;
                    cg = 0;
