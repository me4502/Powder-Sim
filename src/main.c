...
<<<<<<< HEAD
    "\n\brPowder Sim - http://powdertoy.co.uk, irc.freenode.net #powder, #powdersim\n"
=======
...
>>>>>>> upstream/master
    ...
    "\n\n"
    " \boPowder Sim Contributors:\bg Matthew Miller(Me4502), Vanquish349, Eyes, antonvrg\n"
...
void *build_save(int *size, int orig_x0, int orig_y0, int orig_w, int orig_h, unsigned char bmap[YRES/CELL][XRES/CELL], float fvx[YRES/CELL][XRES/CELL], float fvy[YRES/CELL][XRES/CELL], sign signs[MAXSIGNS], void* partsptr)
{
    ...
    {
        i = m[j];
        if (i && (parts[i-1].type==PT_CLNE || parts[i-1].type==PT_PCLN || parts[i-1].type==PT_BCLN || parts[i-1].type==PT_SPRK || parts[i-1].type==PT_LAVA || parts[i-1].type==PT_PIPE || parts[i-1].type==PT_LIFE || parts[i-1].type==PT_NBLE || parts[i-1].type==PT_PBCN || parts[i-1].type==PT_WIRE || parts[i-1].type==PT_STOR || parts[i-1].type==PT_CONV))
            d[p++] = parts[i-1].ctype;
    }
...
<<<<<<< HEAD
    ...
    for (j=0; j<w*h; j++)
    {
        ...
        i = m[j];
        ty = d[pty+j];
        if (i && (ty==PT_CLNE || (ty==PT_PCLN && ver>=43) || (ty==PT_BCLN && ver>=44) || (ty==PT_SPRK && ver>=21) || (ty==PT_LAVA && ver>=34) || (ty==PT_PIPE && ver>=43) || (ty==PT_LIFE && ver>=51) || (ty==PT_NBLE && ver>=51) || (ty==PT_PBCN && ver>=52) || (ty==PT_WIRE && ver>=55) || (ty==PT_STOR && ver>=59) || (ty==PT_CONV && ver>=60)))
        {
            if (p >= size)
                goto corrupt;
            if (i <= NPART)
...
#ifdef LUACONSOLE
        if(!luacon_loadevent(LUACON_SLOAD))
            goto corrupt;
#endif
        if (i && i<=NPART)
        {
