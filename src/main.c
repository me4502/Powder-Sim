...
static const char *it_msg =
    ...
    "\bgTo choose a material, hover over one of the icons on the right,\n it will show a selection of elements in that group.\n"
    ...
    "\n"
    "\boPowder Sim Contributors:\bg Matthew Miller(me4502), Vanquish349, Randalserrano And Antonvrg\n"
    ...
    "\bgSimon Robertshaw, Skresanov Savely, cracker64, Catelite,\n Bryan Hoyle, Nathan Cousins, jacksonmj,"
	"\bgLieuwe Mosch, Anthony Boot,\n Matthew \"me4502\", MaksProg\n"
    "\n"
    "\bgTo use online features such as saving, you need to register at:\n \brhttp://powdertoy.co.uk/Register.html"
    ;
		...
		if (y>0 && y<sdl_scale*YRES && x>0 && x<sdl_scale*XRES)
		{
			...
			if (cr)
			{
				...
				else if ((cr&0xFF)==PT_FIRE && parts[cr>>8].tmp > 0 && parts[cr>>8].tmp < PT_NUM )
				{
					char lowername[6];
					int ix;
					strcpy(lowername, ptypes[parts[cr>>8].tmp].name);
					for (ix = 0; lowername[ix]; ix++)
						lowername[ix] = tolower(lowername[ix]);

					sprintf(nametext, "Burning %s", lowername);
				}
				else if ((cr&0xFF)==PT_SMKE && parts[cr>>8].tmp > 0 && parts[cr>>8].tmp < PT_NUM )
				{
					char lowername[6];
					int ix;
					strcpy(lowername, ptypes[parts[cr>>8].tmp].name);
					...
					sprintf(nametext, "Smoking %s", lowername);
				}
...
#ifdef OGLR
					sprintf(coordtext, "OpenGL Mode: #%d, X:%d Y:%d", cr>>8, x/sdl_scale, y/sdl_scale);
#else
			...
			else
			{
				...
				{
#ifdef OGLR
					sprintf(coordtext, "OpenGL Mode: X:%d Y:%d. GX: %.2f GY: %.2f", x/sdl_scale, y/sdl_scale, gravxf[((y/sdl_scale)*XRES)+(x/sdl_scale)], gravyf[((y/sdl_scale)*XRES)+(x/sdl_scale)]);
#else
