...
void menu_ui(pixel *vid_buf, int i, int *sl, int *sr)
{
	...
	while (!sdl_poll())
	{
		...
		gradient_fill(vid_buf, (XRES-BARSIZE-width)-7, y-10, width+16, height+16+rows,PIXR(gradCol),PIXG(gradCol),PIXB(gradCol),35,0,0,0,1);
		if (i==SC_WALL)
		{
...
void render_ui(pixel * vid_buf, int xcoord, int ycoord, int orientation)
{
	...
	int render_optionicons[] = {0xE4, 0xE2, 0x9B, 0xC4, 0xBF, 0xDE};
