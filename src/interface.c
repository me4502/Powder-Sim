...
{
	int c;
    gradient_fill(vid_buf, 0,YRES+(MENUSIZE-16),XRES + MENUSIZE,MENUSIZE/2+8,90,90,90,255,0,0,0,1);
...
void menu_ui(pixel *vid_buf, int i, int *sl, int *sr)
{
	...
	while (!sdl_poll())
	{
		...
		gradient_fill(vid_buf, (XRES-BARSIZE-width)-7, y-10, width+16, height+16+rows,90,90,90,160,0,0,0,1);
		if (i==SC_WALL)
		{
...
void menu_ui_v3(pixel *vid_buf, int i, int *sl, int *sr, int *dae, int b, int bq, int mx, int my)
{
    ...
    {
        rows = ceil((float)menuitems/16.0f);
        height = (ceil((float)menuitems/16.0f)*18);
        width = restrict_flt(menuitems*31, 0, 16*31);
        fwidth = menuitems*31;
				...
				for (a=1; a<15; a++)
				{
					for (c=1; c<27; c++)
					{
						if (n == DECO_LIGHTEN)
							...
							vid_buf[(XRES+BARSIZE)*(y+a)+((x-xoff)+c)] = PIXRGB(rand()%256, rand()%256, rand()%256);
						else
